...
describe('Access column staleness guard logic', () => {
  // Direct unit coverage of the guard used in workflow_list.tsx's render:
  // the types passed to columns(...) must only reflect resourceSharing.types
  // when it was resolved for the currently selected data source, falling
  // back to [] otherwise. getResourceSharingAvailableTypes(...) is called
  // from a useEffect keyed on dataSourceId, so on a data-source switch,
  // resourceSharing.types can still hold the previous data source's result
